...
USER_PREF_DIR = os.path.expanduser("~/.bibSearch")
...
def parse_options():
    ...
    parser.add_option("-w", "--wiki", dest="wiki", action="store_true",
    ...
    if options.bibtex:
        output_format = 'getBibTexCitation'
    elif options.wiki:
        output_format = 'getWikipediaCitation'
    else:
        output_format = 'getCitation'

    return {'output_format': output_format, 'search_terms': args, 'search_path': BIB_SEARCH_PATH + options.path }
...
_read_config()
from searchconfig import BIB_SEARCH_PATH
